...
def render_summary_insights(df_filtered: pd.DataFrame, df_all: pd.DataFrame) -> None:
    ...
    with col1:
        ...
        st.metric(
            ...
            f"{metrics['positif_count']:,} ({metrics['positif_pct']:.1f}%)",
            delta=f"{positif_delta:+.1f} pp vs overall"
        )
    ...
    with col2:
        ...
        st.metric(
            ...
            f"{metrics['netral_count']:,} ({metrics['netral_pct']:.1f}%)",
            delta=f"{netral_delta:+.1f} pp vs overall"
        )
    ...
    with col3:
        ...
        st.metric(
            ...
            f"{metrics['negatif_count']:,} ({metrics['negatif_pct']:.1f}%)",
            delta=f"{negatif_delta:+.1f} pp vs overall"
        )
...
def render_sentiment_overview(df: pd.DataFrame) -> None:
    ...
    with col1:
        ...
        pie_fig.update_layout(margin=dict(l=10, r=10, t=20, b=20), height=340, showlegend=False)
...
def render_wordcloud_analysis(df: pd.DataFrame) -> None:
    ...
    with col_terms:
        ...
        terms_fig.update_layout(height=380, coloraxis_showscale=False, yaxis_title="", xaxis_title="")
...
def main() -> None:
    ...
    with main_tab3:
        col_wc, col_eval = st.columns([1.2, 1])
