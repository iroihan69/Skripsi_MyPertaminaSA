...
DATA_OPTIONS = {
    "3 Kelas Historis": {
        "prediction_rel": Path("data/predictions/predictions_3kelas_historis.csv"),
        "eval_rel": Path("logs/evaluation_summary_3kelas_historis.json"),
        "model_caption": "🤖 Model: IndoBERT (3-kelas historis)",
    },
    "3 Kelas Baru": {
        "prediction_rel": Path("data/predictions/predictions_3kelas_baru.csv"),
        "eval_rel": Path("logs/evaluation_summary_3kelas_baru.json"),
        "model_caption": "🤖 Model: IndoBERT (3-kelas baru)",
    },
    "2 Kelas Baru": {
        "prediction_rel": Path("data/predictions/predictions_2kelas_baru.csv"),
        "eval_rel": Path("logs/evaluation_summary_2kelas_baru.json"),
        "model_caption": "🤖 Model: IndoBERT (2-kelas real, evaluasi terpisah)",
    },
}
...
def file_sha256(path: str) -> str:
    """Hitung hash file untuk deteksi artefak identik antar mode."""
...
def get_present_sentiment_order(df: pd.DataFrame) -> list[str]:
    """Urutan sentimen yang benar-benar muncul pada data aktif."""
    unique_labels = set(df["label_name"].dropna().astype(str).tolist())
    present = [label for label in SENTIMENT_ORDER if label in unique_labels]
    return present if present else SENTIMENT_ORDER


def find_identical_modes(selected_mode: str, base_dir: Path) -> list[str]:
    """Cari mode lain yang memakai artefak prediksi/evaluasi identik."""
    selected_cfg = DATA_OPTIONS[selected_mode]
    selected_pred_path = base_dir / selected_cfg["prediction_rel"]
    selected_eval_path = base_dir / selected_cfg["eval_rel"]

    selected_pred_hash = file_sha256(str(selected_pred_path)) if selected_pred_path.exists() else ""
    selected_eval_hash = file_sha256(str(selected_eval_path)) if selected_eval_path.exists() else ""

    identical_modes: list[str] = []
    for mode, cfg in DATA_OPTIONS.items():
        if mode == selected_mode:
            continue

        pred_path = base_dir / cfg["prediction_rel"]
        eval_path = base_dir / cfg["eval_rel"]
        pred_hash = file_sha256(str(pred_path)) if pred_path.exists() else ""
        eval_hash = file_sha256(str(eval_path)) if eval_path.exists() else ""

        same_pred = bool(selected_pred_hash) and selected_pred_hash == pred_hash
        same_eval = bool(selected_eval_hash) and selected_eval_hash == eval_hash
        if same_pred or same_eval:
            reasons = []
            if same_pred:
                reasons.append("prediksi")
            if same_eval:
                reasons.append("evaluasi")
            identical_modes.append(f"{mode} ({', '.join(reasons)} identik)")

    return identical_modes
...
def render_sidebar_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    with st.sidebar:
        ...
        st.markdown(f"""
        **📊 Status Filter:**
        - Data yang ditampilkan: {len(filtered_df):,} dari {len(df):,} baris data
...
def render_summary_insights(df_filtered: pd.DataFrame, df_all: pd.DataFrame) -> None:
    ...
    with col_b:
        st.markdown(
            f"""
            <div class="quick-stat"><strong>Data Aktif:</strong> {metrics['total']:,} baris data</div>
            """,
            unsafe_allow_html=True,
        )
...
def render_sentiment_overview(df: pd.DataFrame) -> None:
    ...
    sentiment_order = get_present_sentiment_order(df)
...
def render_wordcloud_analysis(df: pd.DataFrame) -> None:
    ...
    sentiment_options = get_present_sentiment_order(df)
...
def render_prediction_table(df: pd.DataFrame) -> None:
    ...
    st.caption(f"Menampilkan {len(preview_df)} dari {len(df):,} baris data dengan filter aktif")
...
def main() -> None:
    ...
    with st.sidebar:
        selected_mode = st.selectbox("Mode Data & Model", list(DATA_OPTIONS.keys()), index=0)

    selected_config = DATA_OPTIONS[selected_mode]
    prediction_path = base_dir / selected_config["prediction_rel"]
    eval_summary_path = base_dir / selected_config["eval_rel"]

    identical_modes = find_identical_modes(selected_mode, base_dir)
    if identical_modes:
        st.warning(
            "Artefak mode aktif identik dengan mode lain: "
            + "; ".join(identical_modes)
            + ". Perbandingan bisa terlihat sama karena sumber file sama."
        )
    ...
    with col_info1:
        st.caption(f"📊 Total Data: {len(df_all):,} baris data")
    ...
    with col_info3:
        st.caption(str(selected_config["model_caption"]))
